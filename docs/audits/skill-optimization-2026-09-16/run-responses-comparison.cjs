...
// caller-injected credentials AND trusted fetch can dispatch; it grants no consent.
...
const TRANSPORT_LIMITS = Object.freeze({ max_requests: 48, max_output_tokens: 2048, timeout_ms: 60000,
  max_request_bytes: 2097152, max_response_bytes: 1048576 });
class ComparisonError extends Error {
  constructor(code) { super(`Responses comparison: ${code}`); this.name = 'ComparisonError'; this.code = code; }
}
...
function optionsFor(options, allowed) {
  ...
  for (const name of ['sourceRoot', 'proposalPath']) if (options[name] !== undefined && typeof options[name] !== 'string') fail('invalid_options');
  return { model: options.model, effort: options.effort, sourceRoot: path.resolve(options.sourceRoot || ROOT),
...
// Compile only these two fixed trusted repository modules from the exact bytes
...
function freshAuthorModule(relative) {
  if (!['scripts/prepare-skill-pilot.js', 'scripts/lib/skill-responses-transport.js'].includes(relative)) fail('invalid_author_module');
...
const transportModule = freshAuthorModule('scripts/lib/skill-responses-transport.js');
...
function implementationSnapshot(proposal) {
  ...
  return [...proposal.implementation, { path: SELF, sha256: loadedSelfHash }, transportModule.source,
...
function buildInputs(settings, baselineBytes) {
  ...
  const binding = { runtime: { node: process.version, platform: process.platform, arch: process.arch },
    ...
    model_requested: settings.model, effort_requested: settings.effort, cases, sources, order: copy(ORDER),
    driver_limits: { ...DRIVER_LIMITS }, transport_limits: { ...TRANSPORT_LIMITS }, automatic_retries: 0 };
...
async function prepare(settings) {
  ...
  const manifest = { schema_version: 1, kind: 'responses_owner_entry_comparison_preflight', status: 'not_run', execution_authorized: false,
    ...
    limitations: ['This concrete input/configuration freeze is not authorization to send requests.',
      'Model/effort are caller selections, not verified provider availability or identity. Credentials and fetch are absent.',
...
async function preflightComparison(options) {
  const settings = optionsFor(options, ['model','effort','sourceRoot','proposalPath']);
...
async function freezeComparison(options) {
  const settings = optionsFor(options, ['model','effort','sourceRoot','proposalPath','outputPath']);
...
async function runComparison(options) {
  const settings = optionsFor(options, ['model','effort','sourceRoot','proposalPath','manifest','apiKey','fetchImpl','signal']);
  if (!options.manifest || typeof options.fetchImpl !== 'function' || typeof options.apiKey !== 'string'
  ...
  const apiKey = options.apiKey, fetchImpl = options.fetchImpl, external = options.signal;
  if (!apiKey || [...prepared.inputs.values()].some(input => input.publicFiles.some(file => file.bytes.includes(Buffer.from(apiKey))))) fail('invalid_credential_configuration');
  const result = { report: { schema_version: 1, evaluation_kind: 'responses_owner_entry_comparison_smoke', status: 'not_run',
    run_status: 'running', model_execution: null, activation_observed: false, host_isolation_enforced: false,
    ...
    fetch_invocations: 0, stopped_reason: null, transport: null,
    ...
    limitations: ['Only explicitly injected trusted fetch can send requests; no environment credential lookup, global fetch, live CLI, retry, or model fallback exists.',
      'Transport attempts and provider-reported usage are retained even on failure; null/unknown usage is never counted as zero and cost remains unknown.',
      'The shared transport reserves at most 48 requests across all four trials; accepted partial artifacts survive interruption.',
      'Visible structured actions are data only. No model-produced program, shell command, browser action or deployment is executed.',
      'Completed transport/driver flow is not task success or actual Codex Skill activation. Manual rubric items remain unverified.',
      'Injected fetch is trusted JavaScript, not a sandbox; it must honor endpoint, redirect and no-retry requirements.',
      'This historical development smoke requires its frozen source bytes and Git baseline; it is not a permanent CI gate.'] }, artifacts: [] };
  let active = null;
  ...
  const transport = transportModule.exports.createResponsesTransport({ model: settings.model, effort: settings.effort, apiKey,
    limits: TRANSPORT_LIMITS, fetchImpl: async (url, init) => {
      // Last synchronous source/deadline checks immediately before injected fetch.
      try {
        validateSources();
        if (!active || active.closed || init.signal.aborted || external?.aborted) fail('cancelled');
        if (performance.now() >= active.deadline) fail('timed_out');
        if (result.report.fetch_invocations >= TRANSPORT_LIMITS.max_requests) fail('request_limit');
      ...
      result.report.fetch_invocations++;
  ...
  for (const trial of result.report.trials) {
    ...
    const input = prepared.inputs.get(`${trial.case_id}/${trial.variant}`);
    const before = transport.inspect().requests_reserved;
    active = { closed: false, failure: null, deadline: performance.now() + DRIVER_LIMITS.timeout_ms };
    const current = active;
    const execution = await runTaskTrial({ ...input, limits: DRIVER_LIMITS, signal: external,
      adapter: async request => {
        ...
        // Do not allow a response to dispatch a virtual write after source drift.
        try { validateSources(); } catch (error) { current.failure ||= error.code || 'source_drift'; throw new ComparisonError('source_drift'); }
        trial.visible_actions.push({ turn: JSON.parse(request.requestJson).turn, action: JSON.parse(text) });
        return text;
      } });
    current.closed = true;
    trial.driver_report = execution.report;
    ...
    const inspected = transport.inspect();
    trial.attempt_numbers = inspected.attempts.slice(before).map(attempt => attempt.attempt);
    try { validateSources(); trial.provenance_valid = true; }
    catch (error) { current.failure ||= error.code || 'source_drift'; trial.provenance_valid = false; }
  ...
  // Preserve all earlier attempts if the last source check fails; never throw
  // away the usage ledger after requests have been dispatched.
  ...
  result.report.usage_complete = result.report.transport.attempts.length > 0
    && result.report.transport.attempts.every(attempt => attempt.usage_complete);
  result.report.run_status = result.report.stopped_reason ? 'stopped' : 'completed';
  return result;
}

async function cli(argv) {
  if (argv.length === 1 && argv[0] === '--help') {
    console.log('Local only: node run-responses-comparison.cjs --preflight --model <explicit-model> --effort <explicit-effort> --output <new-file>\nNo live CLI exists. Module API: preflightComparison(options), freezeComparison(options), runComparison(options).');
    return;
  }
  if (argv[0] !== '--preflight' || argv.length !== 7) fail('local_preflight_only');
  const parsed = {};
  for (let index = 1; index < argv.length; index += 2) {
    const key = argv[index];
    if (!['--model','--effort','--output'].includes(key) || parsed[key] || !argv[index + 1] || argv[index + 1].startsWith('--')) fail('invalid_options');
    parsed[key] = argv[index + 1];
  }
  const manifest = await freezeComparison({ model: parsed['--model'], effort: parsed['--effort'], outputPath: parsed['--output'] });
  console.log(JSON.stringify({ status: manifest.status, execution_authorized: false, binding_sha256: manifest.binding_sha256, trials: 4, model_requests: 0 }));
}
if (require.main === module) cli(process.argv.slice(2)).catch(error => { console.error(error instanceof ComparisonError ? error.message : 'Responses comparison: local_preflight_failed'); process.exitCode = 1; });
