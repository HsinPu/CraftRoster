...
// Author-only native request_user_input boundary probe. The loopback provider
// supplies one fixed synthetic question call, then a terminal 400. No model,
// credential, forwarded traffic, file tool, shell, or external action is involved.
...
const OUTPUT = path.join(__dirname, 'codex-tool-surface-probe-v6.json');
...
const QUESTION_CALL = Object.freeze({ type: 'function_call', id: 'fc_author_question_boundary', call_id: 'author_question_boundary',
  name: 'request_user_input', status: 'completed', arguments: JSON.stringify({ questions: [{ id: 'author_probe', header: 'Probe',
    question: 'Synthetic local boundary check; no real user input is requested.',
    options: [{ label: 'A', description: 'Fixed author option A.' }, { label: 'B', description: 'Fixed author option B.' }] }] }) });
const AUTHOR_EVENTS = [
  ['response.created', { type: 'response.created', response: { id: 'resp_author_boundary', object: 'response', status: 'in_progress', output: [] } }],
  ['response.output_item.added', { type: 'response.output_item.added', output_index: 0, item: { ...QUESTION_CALL, status: 'in_progress', arguments: '' } }],
  ['response.function_call_arguments.delta', { type: 'response.function_call_arguments.delta', output_index: 0, item_id: QUESTION_CALL.id, delta: QUESTION_CALL.arguments }],
  ['response.function_call_arguments.done', { type: 'response.function_call_arguments.done', output_index: 0, item_id: QUESTION_CALL.id, arguments: QUESTION_CALL.arguments }],
  ['response.output_item.done', { type: 'response.output_item.done', output_index: 0, item: QUESTION_CALL }],
  ['response.completed', { type: 'response.completed', response: { id: 'resp_author_boundary', object: 'response', status: 'completed', output: [QUESTION_CALL],
...
function summarizeRequest(bytes) {
  ...
  const functionOutputs = input.filter(item => item?.type === 'function_call_output').slice(0, 8).map(item => {
    const output = typeof item.output === 'string' ? item.output : JSON.stringify(item.output ?? null);
    return { call_id: safeToken(item.call_id), bytes: Buffer.byteLength(output), sha256: hash(output),
      unavailable_in_default_mode: /request_user_input[^\n]{0,120}unavailable[^\n]{0,120}default mode/i.test(output),
      argument_validation_error: /invalid arguments|failed to parse|missing field|unknown field/i.test(output) };
  });
  ...
  return { json_valid: true, bytes: bytes.length, sha256: hash(bytes),
    ...
    input_items: input.length, input_summary_truncated: input.length > 256, messages, tools, function_outputs: functionOutputs,
...
async function main() {
  ...
  const report = {
    ...
    source_version: 6,
    previous_probe: { report: 'codex-tool-surface-probe-v5.json', harness_sha256: '4f86416c357853abfe27b3a941c402da1e1d7d75f8c01862b2f8885d79fd0bf9',
      outcome: 'V5 removed both known system Skill names but still advertised request_user_input. V6 sends exactly one authored native question call to observe its runtime boundary.' },
    ...
    authored_response: { kind: 'fixed_synthetic_function_call', name: QUESTION_CALL.name, call_id: QUESTION_CALL.call_id,
      arguments_sha256: hash(QUESTION_CALL.arguments), sse_sha256: hash(AUTHOR_EVENTS), synthetic_usage_is_not_model_usage: true },
    authored_response_sent: false,
    ...
    limitations: [
      'The only supplied action is an authored synthetic question call, not a model-generated response. This cannot assess Skill activation or task success.',
      'The script never forwards requests. Configuration, allowlisted environment, and one native tool rejection do not constitute OS-level network or filesystem isolation.',
  ...
  try {
    server = http.createServer({ maxHeaderSize: 8192 }, (req, res) => {
      ...
      req.on('end', () => {
        ...
        if (item.ordinal === 1 && item.route === 'responses' && item.body?.json_valid) {
          item.response_status = 200; report.authored_response_sent = true;
          res.writeHead(200, { 'Content-Type': 'text/event-stream', Connection: 'close' }); res.end(AUTHOR_EVENTS);
        } else reject(res);
