...
const output = path.join(__dirname, 'codex-task-adapter-probe-v4.json');
...
async function main() {
  ...
  const report = { schema_version: 1, evaluation_kind: 'authored_codex_loopback_task_adapter', status: 'not_run',
    ...
    previous_probe: { report: 'codex-task-adapter-probe-v3.json', outcome: 'The startup error item remains after suppressing unstable feature warnings. V4 captures only bounded redacted error-item diagnostics; no parser rule is relaxed.' },
