...
const output = path.join(__dirname, 'codex-task-adapter-probe-v6.json');
...
async function main() {
  ...
  const report = { schema_version: 1, evaluation_kind: 'authored_codex_loopback_task_adapter', status: 'not_run',
    ...
    previous_probe: { report: 'codex-task-adapter-probe-v5.json', outcome: 'V5 completed all four authored actions. V6 verifies the reviewed transport requiring thread.started and retaining known usage when bounded stdout accompanies oversized stderr.' },
