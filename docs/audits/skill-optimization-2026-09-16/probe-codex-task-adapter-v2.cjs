...
const output = path.join(__dirname, 'codex-task-adapter-probe-v2.json');
...
async function main() {
  ...
  const report = { schema_version: 1, evaluation_kind: 'authored_codex_loopback_task_adapter', status: 'not_run',
    ...
    previous_probe: { report: 'codex-task-adapter-probe.json', outcome: 'The first request exposed no native tools. The original probe incorrectly expected request_user_input and deliberately returned 400. V2 requires an empty tool list.' },
  ...
  try {
    ...
    const config = { model_provider:'author_loopback_probe', model, model_reasoning_effort:effort,
