...
function buildReadiness() {
  ...
  const cases = registry.category_representatives.map(representative => {
    ...
    if (skill === 'mcp-creator-design') {
      ...
      record.limitations.push('The lockfile and offline author document replay are packaged. Dependencies must be provisioned and verified in each trial workspace, and a host must supply the documented bounded command capability; the current virtual-file broker alone cannot execute it. Historical starting-project checks are not the missing server implementation or a model task result.');
    }
  ...
  return { schema_version: 1, evaluation_kind: 'offline_category_input_readiness', recorded_at: new Date().toISOString(),
    ...
    sources: [
      { path: 'docs/audits/skill-optimization-2026-09-16/mcp-starting-project-verification.json', sha256: sha(read('docs/audits/skill-optimization-2026-09-16/mcp-starting-project-verification.json')) },
...
if (require.main === module) {
  ...
  const report = buildReadiness(), output = path.join(__dirname, 'm4-category-readiness-v2.json');
