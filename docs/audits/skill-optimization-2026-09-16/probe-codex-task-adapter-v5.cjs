...
const output = path.join(__dirname, 'codex-task-adapter-probe-v5.json');
...
async function main() {
  ...
  const report = { schema_version: 1, evaluation_kind: 'authored_codex_loopback_task_adapter', status: 'not_run',
    ...
    previous_probe: { report: 'codex-task-adapter-probe-v4.json', outcome: 'V4 identified the exact nonfatal startup notice for deliberately disabled Code Mode. V5 uses the parser with a single exact pre-turn notice allowance; all other error and native-tool rejection remains.' },
  ...
  try {
    ...
    async function executeTurn(input) {
      ...
      record.startup_error_items=stdout.split(/\r?\n/).flatMap(line=>{try{const e=JSON.parse(line);if(e.type!=='item.completed'||e.item?.type!=='error')return [];let text=String(e.item.message??e.item.text??'');for(const value of [scratch,root,executable])text=text.replaceAll(value,'<redacted-path>');return [{keys:Object.keys(e.item).filter(k=>/^[a-z_]{1,64}$/.test(k)),message:text.slice(0,512)}];}catch{return [];}}).slice(0,4);
