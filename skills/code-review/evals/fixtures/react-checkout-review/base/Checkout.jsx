...
export function Checkout() {
  ...
  useEffect(() => {
    if (view.status === 'error') errorRef.current?.focus();
  }, [view.status]);
    ...
    <main>
      ...
      <div className="checkout-grid">
        ...
        <section aria-labelledby="summary-heading">
          ...
          <p role="status" aria-live="polite">{view.status === 'loading' ? 'Loading shipping quote…' : view.status === 'ready' ? 'Quote ready' : ''}</p>
          {view.status === 'error' && (
            <div className="error" role="alert" tabIndex={-1} ref={errorRef}>
              <p>{view.error}</p>
              <button type="button" onClick={() => controller.retry()}>Retry quote</button>
            </div>
          )}
